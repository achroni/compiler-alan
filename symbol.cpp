...
static struct Type_tag typeConst [] = {
    ...
    { TYPE_STRING,    NULL, 0, 0 },
    { TYPE_CHAR,    NULL, 0, 0 },
    { TYPE_REAL,    NULL, 0, 0 },
    { TYPE_BYTE,    NULL, 0, 0 },
    { TYPE_ARRAY_INTEGER,    NULL, 0, 0 },
    { TYPE_ARRAY_BYTE,    NULL, 0, 0 }
};

const MyType typeVoid    = &(typeConst[0]);
const MyType typeInteger = &(typeConst[1]);
const MyType typeBoolean = &(typeConst[2]);
const MyType typeString = &(typeConst[3]);
const MyType typeChar    = &(typeConst[4]);
const MyType typeReal    = &(typeConst[5]);
const MyType typeByte    = &(typeConst[6]);
const MyType typeArrayInteger = &(typeConst[7]);
const MyType typeArrayByte    = &(typeConst[8]);
...
SymbolEntry * newVariable (const char * name, MyType type)
{
    ...
    //printf ("%s : %d with nesting level = %d and offset = %d\n",e->id,e->u.eVariable.type,e->nestingLevel,e->u.eVariable.offset);
    return e;
}
    ...
    if (e == NULL) {
        ...
        if (e != NULL) {
           ...
           // e->u.eFunction.isForward = false;
            ...
            e->u.eFunction.resultType = type;
            e->u.eFunction.funcNode = node;
        }
        return e;
    }
